...
public:
	...
	HiddenNeuron const& hiddenActivationFunction(size_t layer)const{
    RANGE_CHECK(layer < m_hiddenNeurons.size());
		return m_hiddenNeurons[layer];
	}
	...
	/// \brief Returns the activation function of the hidden units.
	HiddenNeuron& hiddenActivationFunction(size_t layer){
    RANGE_CHECK( layer < m_hiddenNeurons.size() );
		return m_hiddenNeurons[layer];
	}
	/// \brief Returns the activation function of the output units.
	...
	void evalLayer(std::size_t layer,RealMatrix const& patterns,RealMatrix& outputs)const{
		...
		if(layer < m_layerMatrix.size()-1) {
			noalias(outputs) = m_hiddenNeurons[layer](outputs);
		}
		else {
	...
	void eval(RealMatrix const& patterns,RealMatrix& output, State& state)const{
		...
		for(std::size_t layer = 0; layer != m_layerMatrix.size();++layer){
			...
			SHARK_CRITICAL_REGION{//beware Dropout Neurons!
				...
				if(layer < m_layerMatrix.size()-1) {
					noalias(responses) = m_hiddenNeurons[layer](responses);
				}
				else {
    ...
    std::size_t numHiddenLayers = layers.size() - 2;
    m_hiddenNeurons.resize(numHiddenLayers);
...
private:
	...
	void computeDelta(
		...
		while(endNeuron > endIndex){
			...
			if(layer != 0){
				noalias(layerDelta) *= m_hiddenNeurons[layer].derivative(layerResponse);
			}
	...
	std::vector<HiddenNeuron> m_hiddenNeurons;
