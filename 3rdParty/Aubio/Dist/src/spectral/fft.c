...
aubio_fft_t * new_aubio_fft (uint_t winsize) {
  ...
  const int flags = IPP_FFT_NODIV_BY_ANY;
  
  int sizeSpec, sizeInit, sizeBuffer;
#if HAVE_AUBIO_DOUBLE
  IppStatus status = ippsFFTGetSize_R_64f(order, flags, qualityHint,
      &sizeSpec, &sizeInit, &sizeBuffer);
#else
  IppStatus status = ippsFFTGetSize_R_32f(order, flags, qualityHint,
    &sizeSpec, &sizeInit, &sizeBuffer);
#endif
...
#else
  // apply fft
  ippsFFTInv_CCSToR_32f(s->complexOut, output->data, s->fftSpec, s->memBuffer);
  // apply scaling
